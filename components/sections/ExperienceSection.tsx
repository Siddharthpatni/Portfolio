...
export const ExperienceSection: React.FC = () => {
  return (
    <section id="experience" aria-label="Professional experience" className="py-16 sm:py-24 px-4 sm:px-6 relative">
      <div className="max-w-7xl mx-auto">
        <SectionHeader
          number="SEC_02"
          title="EXPERIENCE // ENGINEERING TIMELINE"
          subtitle="Production systems shipped, research conducted, and teams contributed to."
        />
        ...
        <div className="mt-10 sm:mt-12 relative border-l border-white/5 pl-4 sm:pl-6 ml-2 sm:ml-4 space-y-8 sm:space-y-12">
          {experiences.map((exp) => (
            <div key={exp.id} className="relative group">
              ...
              <span className="absolute -left-[23px] sm:-left-[31px] top-1.5 flex h-3 w-3 sm:h-4 sm:w-4" aria-hidden="true">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-3 sm:pb-4 mb-3 sm:mb-4">
                  <div>
                    <h3 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
                      <Briefcase className="w-4 h-4 text-spidey-red shrink-0" aria-hidden="true" />
                      <span>{exp.role}</span>
                    </h3>
                ...
                <ul className="space-y-2 mb-4 sm:mb-6">
                  {exp.description.map((bullet, idx) => (
                    <li key={idx} className="text-gray-300 text-xs sm:text-sm leading-relaxed flex items-start gap-2">
                      <span className="text-spidey-red font-mono font-bold mt-0.5 shrink-0" aria-hidden="true">→</span>
                      <span>{bullet}</span>
                    </li>
                  ))}
                </ul>
